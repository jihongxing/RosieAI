...
截至 2026-04-29，工程实现采用“通信技术链路”和“业务层 MVP”分线推进：

| 模块 | 当前状态 | 说明 |
|------|----------|------|
| call-webhook | 已跑通 | 公网 webhook、AI greeting、接入号映射、`say + listen + hangup` verbs 已验证 |
| ai-agent | 已跑通 | 当前服务器资源较小，默认走 DeepSeek / OpenAI-compatible API，本地 Qwen 作为后续降本方向 |
| realtime-voice | 骨架已跑通 | WebSocket 服务和公网健康检查通过，已准备承接 jambonz `listen` 音频流 |
| 业务摘要 | 已跑通模拟链路 | `/simulate/call-result` 可将模拟转写写入摘要、收件箱和汇总 |
...
| 通知 | 小程序收件箱 + 微信订阅消息定时汇总 | 企业微信群机器人、企业微信应用 / 客服、钉钉、短信兜底 | MVP 默认少打扰，定时给用户决策清单 |
| 通信线路 | 三大运营商政企 IMS / SIP 语音专线 | 有资质线路集成商 | 必须签清楚 SLA，并验证条件呼叫转移到 Rosie 幕后接入号 |
| 短信 | 腾讯云短信 | 阿里云短信、华为云短信 | 仅用于必要兜底 |

通知方案定型：

- Phase 2 先建设小程序收件箱，保存全量来电摘要、分类、转写和回拨建议。
- 默认每日 20:00 通过微信订阅消息发送汇总，可选午间 12:00 + 晚间 20:00 两次汇总。
- 实时通知默认关闭，只对投诉、紧急事项、明确要求回电等例外场景开启。
- 企业微信群机器人 Webhook 只作为团队商家可选通知，不作为默认开通步骤。
...
11. 通话结束后 Rosie 生成摘要，写入小程序收件箱，并进入定时汇总
```
...
| Notification Service | 小程序收件箱、定时汇总、微信订阅消息、企业微信群机器人、企业微信应用 / 客服、钉钉、短信兜底 |
...
- 小程序收件箱写入失败进入重试队列。
- 汇总任务失败进入重试队列。
- 微信订阅消息、企业微信群机器人等外部通知失败记录到 `notification_logs`，不影响收件箱主数据。
...
- 建立通话转写、摘要、意图、收件箱、汇总和通知偏好数据结构。
- 增加模拟转写入口 `/simulate/call-result`。
- 调用 AI 提取来电摘要和结构化字段。
- 建立小程序收件箱后端接口 `/inbox`。
- 建立汇总预览、正式生成和历史查询接口。
